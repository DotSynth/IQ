...
namespace IQ.Views.AdminViews.Pages.Sales
{
    /// <summary>
    /// An empty page that can be used on its own or navigated to within a Frame.
    /// </summary>
    public sealed partial class CompanySalesPage : Page
    {
        ...
        private void CompanySalesAutoSuggestBox_SuggestionChosen(AutoSuggestBox sender, AutoSuggestBoxSuggestionChosenEventArgs args)
        {
        ...
        private async void CompanySalesAutoSuggestBox_QuerySubmitted(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
        {
            if (!string.IsNullOrWhiteSpace(args.QueryText))
            {
                // Perform a database query based on the user's queryText
                string userQuery = args.QueryText;
                ObservableCollection<CompanySale> searchResults = await DatabaseExtensions.QueryCompanySaleResultsFromDatabase(userQuery);
            ...
            {
                UpdateSalesPageWithResults(ViewModel.CompanySales);
            }
        }

        private void UpdateSalesPageWithResults(ObservableCollection<CompanySale> searchResults)
        {
            try
            {
                // Assuming that BranchSalesDataGrid is the name of your DataGrid
                CompanySalesDataGrid.ItemsSource = searchResults;
            }
            catch (Exception ex)
            {
        ...
        private async void CompanySalesAutoSuggestBox_TextChanged(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
        {
            if (args.Reason == AutoSuggestionBoxTextChangeReason.UserInput)
            {
                // Query the database for suggestions based on the user's input
                string userInput = sender.Text;
                List<string> suggestions = await DatabaseExtensions.QueryCompanySalesSuggestionsFromDatabase(userInput);
        ...
        private void CompanySalesDatePicker_SelectedDateChanged(DatePicker sender, DatePickerSelectedValueChangedEventArgs args)
        {
            DateFilter = CompanySalesDatePicker.Date.UtcDateTime;
            RefreshPage();
        }
        ...
        public async void RefreshPage()
        {
            ...
            Frame.Navigate(typeof(CompanySalesPage));
        }
        ...
        private async Task LoadSuggestionsAsync()
        {
            try
            {
                ...
                CompanySalesAutoSuggestBox.ItemsSource = suggestions;
            }
            catch (Exception ex)
            {
