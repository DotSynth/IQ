...
namespace IQ.Views.BranchViews.Pages.TransferOutwards
{
    ...
    public sealed partial class TransferOutwardsPage : Page
    {
        ...
        public TransferOutwardsPage()
        {
            ...
            BranchTOutsDatePicker.SelectedDate = DateFilter;
            BranchTOutsDatePicker.MaxYear = DateTime.UtcNow.Date;
        ...
        private void BranchTOutsDatePicker_SelectedDateChanged(DatePicker sender, DatePickerSelectedValueChangedEventArgs args)
        {
            DateFilter = BranchTOutsDatePicker.Date.UtcDateTime;
            RefreshPage();
        }
        ...
        private async Task LoadSuggestionsAsync()
        {
            try
            {
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(StructureTools.BytesToIQXFile(File.ReadAllBytes(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, LoginWindow.User))).ConnectionString))
                {
                    ...
                    using (NpgsqlCommand command = new NpgsqlCommand("SELECT DISTINCT TransferID FROM BranchTransferOutwards WHERE DATE(Date) = @time;", connection))
                    {
                ...
                BranchTOutsAutoSuggestBox.ItemsSource = suggestions;
            }
            catch (Exception ex)
            {
        ...
        private void BranchTOutsAutoSuggestBox_SuggestionChosen(AutoSuggestBox sender, AutoSuggestBoxSuggestionChosenEventArgs args)
        {
        ...
        private async void BranchTOutsAutoSuggestBox_QuerySubmittedAsync(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
        {
            if (!string.IsNullOrWhiteSpace(args.QueryText))
            {
                ...
                ObservableCollection<BranchTOut> searchResults = await DatabaseExtensions.QueryTOutsResultsFromDatabase(userQuery);
                Debug.WriteLine("PopupPageVisibilityChanged called");
        ...
        private void UpdateTOutsPageWithResults(ObservableCollection<BranchTOut> searchResults)
        {
            try
            {
                ...
                BranchTOutsDataGrid.ItemsSource = searchResults;
            }
            catch (Exception ex)
            {
        ...
        private async void BranchTOutsAutoSuggestBox_TextChangedAsync(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
        {
            if (args.Reason == AutoSuggestionBoxTextChangeReason.UserInput)
            {
                // Query the database for suggestions based on the user's input
                string userInput = sender.Text;
                List<string> suggestions = await DatabaseExtensions.QueryTOutsSuggestionsFromDatabase(userInput);
