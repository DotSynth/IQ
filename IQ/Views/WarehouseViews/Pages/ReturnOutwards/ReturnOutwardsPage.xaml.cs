...
using IQ.Views.WarehouseViews.Pages.ReturnOutwards.SubPages;
...
namespace IQ.Views.WarehouseViews.Pages.ReturnOutwards
{
    /// <summary>
    ...
    public sealed partial class ReturnOutwardsPage : Page
    {
        ...
        private void WarehouseROutsDatePicker_SelectedDateChanged(DatePicker sender, DatePickerSelectedValueChangedEventArgs args)
        {
            DateFilter = WarehouseROutsDatePicker.Date.UtcDateTime;
            RefreshPage();
        }
        ...
        private async Task LoadSuggestionsAsync()
        {
            try
            {
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(StructureTools.BytesToIQXFile(File.ReadAllBytes(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, LoginWindow.User))).ConnectionString))
                {
                    ...
                    using (NpgsqlCommand command = new NpgsqlCommand("SELECT DISTINCT ReturnID FROM WarehouseReturnOutwards WHERE DATE(Date) = @time;", connection))
                    {
                ...
                WarehouseROutsAutoSuggestBox.ItemsSource = suggestions;
            }
            catch (Exception ex)
            {
        ...
        private void WarehouseROutsAutoSuggestBox_SuggestionChosen(AutoSuggestBox sender, AutoSuggestBoxSuggestionChosenEventArgs args)
        {
        ...
        private async void WarehouseROutsAutoSuggestBox_QuerySubmittedAsync(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
        {
            if (!string.IsNullOrWhiteSpace(args.QueryText))
            {
                ...
                ObservableCollection<WarehouseROut> searchResults = await DatabaseExtensions.QueryWHROutsResultsFromDatabase(userQuery);
                Debug.WriteLine("PopupPageVisibilityChanged called");
        ...
        private void UpdateROutsPageWithResults(ObservableCollection<WarehouseROut> searchResults)
        {
            try
            {
                ...
                WarehouseROutsDataGrid.ItemsSource = searchResults;
            }
            catch (Exception ex)
            {
        ...
        private async void WarehouseROutsAutoSuggestBox_TextChangedAsync(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
        {
            if (args.Reason == AutoSuggestionBoxTextChangeReason.UserInput)
            {
                // Query the database for suggestions based on the user's input
                string userInput = sender.Text;
                List<string> suggestions = await DatabaseExtensions.QueryWHROutsSuggestionsFromDatabase(userInput);
