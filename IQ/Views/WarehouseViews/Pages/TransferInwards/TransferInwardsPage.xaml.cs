...
namespace IQ.Views.WarehouseViews.Pages.TransferInwards
{
    ...
    {
        public WHTInsViewModel ViewModel { get; } = new WHTInsViewModel();
        ...
        private async Task LoadSuggestionsAsync()
        {
            try
            {
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(StructureTools.BytesToIQXFile(File.ReadAllBytes(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, LoginWindow.User))).ConnectionString))
                {
                    ...
                    using (NpgsqlCommand command = new NpgsqlCommand("SELECT DISTINCT TransferID FROM WarehouseTransferInwards WHERE DATE(Date) = @time;", connection))
                    {
        ...
        private async void WarehouseTInsAutoSuggestBox_QuerySubmittedAsync(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
        {
            ...
            {
                UpdateTInsPageWithResults(ViewModel.WarehouseTIn);
            }
        }
